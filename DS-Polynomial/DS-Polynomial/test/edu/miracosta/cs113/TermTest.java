...
public class TermTest {
	...
	// if TERMS array changed, make sure to update COEFFICIENTS and/or EXPONENTS
	// array
	private static final String[] TERMS = { "", "+5", "-3", "+x", "-x", "+6x", "-9x", "+x^3", "-x^5", "+x^-2", "-x^-7",
			"+7x^4", "-2x^13", "+25x^-8", "-54x^-17" };
	...
	public void testCopyConstructor() {
		...
		assertEquals("Coefficients in 'copy' does not match 'original'", original.getCoefficient(),
				copy.getCoefficient());
		assertEquals("Exponents in 'copy' does not match 'original'", original.getExponent(), copy.getExponent());
		assertTrue("Reference is not a unique memory address (shallow copy)", original != copy);
	}
	...
	public void testCompareTo() {
		...
		for (int i = 0; i < EXPONENTS.length; i++) {
			...
			if (COMPARE_TO_VALUES[i] == ORIGINAL_EQUALS_TEST) {
				assertEquals(
						"Expected and actual compareTo result (original exponent EQUALS test exponent) DON'T match", 0,
						compareResult);
